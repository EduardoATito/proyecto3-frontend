...
export class SidebarComponent implements OnInit {
  ...
  toggleDropdown(category: string) {
    // for (const category in this.dropdownStates) {
    //   if (category !== category) {
    //     this.dropdownStates[category] = false;
    //   }
    // }
    this.dropdownStates[category] = !this.dropdownStates[category];
  }


  verificarRol(){
    const rol = this.jwtService.getRol();
    if(rol === Roles.ADMIN){
      this.menuItems = this.menuItemsAdmin;
    }else if(rol === Roles.AYUDANTE){
      this.menuItems = this.menuItemsAyudantes;
    }
  }
}
